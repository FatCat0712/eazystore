...
export default function CheckoutForm() {
  ...
  const getClassForElement = (field) => `${fieldBaseClass} ${elementErrors[field] ? fieldErrorClass : fieldValidClass}`;
  ...
  const elementOptions = {
    style: {
      base: {
        ...
        backgroundColor: isDarkMode ? "#4B5563" : "#FFFFF",
      },
      invalid: {
        color: "#F87171",
  ...
  const handleSubmit = async (event) => {
    ...
    try{
      ...
      setErrorMessage("Error proccessing payment. Please try again later.");
  ...
  return (
    <div className='min-h-[852px] flex items-center justify-center font-primary dark:bg-darkbg'>
      <div className={isProcessing ? 'visibile flex flex-col justify-center items-center my-[200px]' : "hidden"}>
      ...
      <div className={isProcessing ? "hidden" : "visible bg-white dark:bg-gray-700 shadow-md rounded-lg max-w-md w-full px-8 py-6"}>
          ...
          <form onSubmit={handleSubmit} className='space-y-6'>
            ...
            {/* Card Expiry */}
            <div>
              <label htmlFor="cardExpiry" className={labelStyle}>Expiry Date</label>
