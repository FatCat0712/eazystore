...
export default function Contact() {
  ...
  function handleSubmit(event) {
    ...
    const userConfirmed = window.confirm('Are you sure you wan to submit the form?');

    if(userConfirmed) {
      const formData = new FormData(formRef.current); // Get form data
      submit(formData, {method: 'POST'}); // Proceed with form with submission
    }
    else {
  ...
  const textFieldStyle = 'w-full px-4 py-2 text-base border rounded-md transition border-primary dark:border-light focus:ring dark:focus:ring-lighter' + 
   'focus:outline-none text-gray-800 dark:text-lighter bg-white dark:bg-gray-600 placeholder-gray-400 dark:placeholder-gray-300';
  return (
     <div className="max-w-[1152px] min-h-[852px] mx-auto px-6 py-8 font-primary bg-normalbg dark:bg-darkbg">
      ...
      {/* Contact Form */}
      <Form method="POST" onSubmit={handleSubmit}  className="space-y-6 max-w-[768px] mx-auto" ref={formRef}>
        {/* Name Field */}
        <div>
